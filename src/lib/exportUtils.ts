...
//     const dataRows: any[][] = (reportList || []).map((item) => {
//       const isSale = item.type === "SALE";
//       return [
//         formatDate(new Date(item.createdAt)),
//         isSale
...
//     const filename = `Financial_Report_${format(new Date(), "yyyyMMdd")}.xlsx`;
...
//           await navigator.share({
//             files: [file],
//             title: pt("analytics.reports.shareExcel", "Share as Excel"),
//           });
//         } catch (shareErr) {
//           console.warn("Sharing failed, falling back to download", shareErr);
//           XLSX.writeFile(wb, filename);
