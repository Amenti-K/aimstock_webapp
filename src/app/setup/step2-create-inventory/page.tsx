...
import { inventoriesArraySchema, inventoriesArrayFormValues } from "@/components/schema/inventory.schema";
...
export default function Step2InventoryPage() {
  ...
  const { data: warehousesResponse, isLoading: loadingWarehouses } = useFetchWarehouseSelector();
  const warehouses = warehousesResponse?.data || [];

  const { mutateAsync: createBulkInventory, isPending } = useCreateBulkInventory();
  ...
  React.useEffect(() => {
    if (warehouses.length > 0 && fields.length > 0 && fields[0].warehouseInventories.length === 0) {
      form.setValue("inventories.0.warehouseInventories", warehouses.map(w => ({
        warehouseId: w.id,
        quantity: 0,
        reorderQuantity: 0,
      })));
  ...
  const onSubmit = async (data: inventoriesArrayFormValues) => {
    try {
      ...
      router.push("/app/setup/step3-create-partners");
    } catch (error) {
      console.error(error);
    }
  };
    ...
    <div className="w-full max-w-4xl mx-auto py-8">
      <div className="mb-8 scale-in-center">
        ...
        <p className="text-muted-foreground mt-2">
          Let's add some initial inventory to your system. You can specify prices, quantities and distribute them across your warehouses.
        </p>
          ...
          {fields.map((field, index) => (
            <Card key={field.id} className="fade-in-up border-primary/20 bg-background shadow-sm">
              <CardContent className="p-6 relative">
                ...
                <h3 className="text-lg font-semibold mb-4 text-primary">Product #{index + 1}</h3>
                ...
                <Accordion type="single" collapsible className="w-full">
                  <AccordionItem value="warehouses" className="border-none">
                    ...
                    <AccordionContent className="pt-4 px-2 space-y-4">
                      {form.watch(`inventories.${index}.warehouseInventories`)?.map((wi, wIndex) => {
                        const warehouse = warehouses.find(w => w.id === wi.warehouseId);
                        return (
                          <div key={wIndex} className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center bg-background border rounded-lg p-3">
                            <span className="font-semibold text-sm">{warehouse?.name || "Unknown Warehouse"}</span>
                            <div className="flex space-x-2">
                              ...
                              <FormField
                                ...
                                render={({ field }) => (
                                  <FormItem className="flex-1">
                                    <FormLabel className="text-xs">Reorder At</FormLabel>
                                    <FormControl>
                                      <Input type="number" {...field} onChange={e => field.onChange(parseInt(e.target.value) || 0)} />
                                    </FormControl>
                                  </FormItem>
                ...
                {form.formState.errors?.inventories?.[index]?.initialQuantity && (
                  <p className="text-sm font-medium text-destructive mt-2">
                    {form.formState.errors.inventories[index]?.initialQuantity?.message}
                  </p>
                )}
