...
const defaultValues: SalesFormValues = {
  ...
  salePayments: [{ accountId: "", amount: 0, description: "" }],
};
...
export default function SalesForm({
  ...
  const form = useForm<SalesFormValues>({ defaultValues });
    ...
    <Form {...form}>
      <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end">
          <SubmitButton
            title={initialData ? "Update Sale" : "Create Sale"}
            loading={isLoading}
            className="w-auto"
          />
        </div>
